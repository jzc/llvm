...
// RUN: %if asserts %{sycl-post-link -properties -json=0 -debug-only=SpecConst %t.bc -spec-const=native -o %t.txt 2>&1 | FileCheck %s -check-prefixes=CHECK-LOG %} %else %{sycl-post-link -json=0 %t.bc -properties -spec-const=native -o %t.txt 2>&1 %}
