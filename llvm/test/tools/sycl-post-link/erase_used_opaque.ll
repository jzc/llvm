...
;
; RUN: sycl-post-link -properties -json=0 -split=kernel -S < %s -o %t.files.table
; RUN: FileCheck %s -input-file=%t.files_0.ll
; RUN: FileCheck %s -input-file=%t.files_1.ll
;
; RUN: sycl-post-link -properties -json=0 -S -split=auto -symbols -split-esimd -lower-esimd -O2 -spec-const=emulation < %s -o %t.out.table
