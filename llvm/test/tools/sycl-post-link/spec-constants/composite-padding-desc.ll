; RUN: sycl-post-link -properties -json=0 -spec-const=native < %s -o %t.files.table
; RUN: FileCheck %s -input-file=%t.files_0.prop
; RUN: %if asserts %{ sycl-post-link -properties -json=0 -debug-only=SpecConst -spec-const=native < %s 2>&1 | FileCheck %s --check-prefix=CHECK-LOG %}
;
