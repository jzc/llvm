; RUN: sycl-post-link -properties -json=0 -spec-const=emulation < %s -S -o %t.table
...
; RUN: %if asserts %{ sycl-post-link -properties -json=0 -debug-only=SpecConst -spec-const=emulation < %s -S 2>&1 | FileCheck %s --check-prefixes=CHECK-LOG,CHECK-LOG-EMULATION %}
; RUN: %if asserts %{ sycl-post-link -properties -json=0 -debug-only=SpecConst -spec-const=native < %s -S 2>&1 | FileCheck %s --check-prefixes=CHECK-LOG,CHECK-LOG-NATIVE %}
