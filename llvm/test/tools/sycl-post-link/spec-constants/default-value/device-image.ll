...
; RUN: sycl-post-link -properties -json=0 -split=auto -spec-const=native -symbols -S -o %t.table %s -generate-device-image-default-spec-consts
...
; RUN: %if asserts %{ sycl-post-link -properties -json=0 -debug-only=SpecConst -split=auto -spec-const=native -symbols -S %s -generate-device-image-default-spec-consts 2>&1 | FileCheck %s --check-prefix=CHECK-LOG %}
