...
;
; Global scope
; RUN: sycl-post-link -properties -json=0 -symbols -emit-exported-symbols -S < %s -o %t.global.files.table
; RUN: FileCheck %s -input-file=%t.global.files_0.prop --implicit-check-not="NotExported" --check-prefix=CHECK-GLOBAL-PROP
;
; Per-module split
; RUN: sycl-post-link -properties -json=0 -symbols -split=source -emit-exported-symbols -S < %s -o %t.per_module.files.table
...
;
; Per-kernel split
; RUN: sycl-post-link -properties -json=0 -symbols -split=kernel -emit-exported-symbols -S < %s -o %t.per_kernel.files.table
