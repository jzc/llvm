...
; Default lowering
; RUN: sycl-post-link -properties -json=0 -split-esimd -lower-esimd -S < %s -o %t.table
; RUN: FileCheck %s -input-file=%t_esimd_0.ll --check-prefixes CHECK-O2

; -O2 lowering
; RUN: sycl-post-link -properties -json=0 -split-esimd -lower-esimd -O2 -S < %s -o %t.table
; RUN: FileCheck %s -input-file=%t_esimd_0.ll --check-prefixes CHECK-O2

; -O0 lowering
; RUN: sycl-post-link -properties -json=0 -split-esimd -lower-esimd -O0 -S < %s -o %t.table
