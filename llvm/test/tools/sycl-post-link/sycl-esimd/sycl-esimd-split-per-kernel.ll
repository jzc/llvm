; RUN: sycl-post-link -properties -json=0 -split-esimd -split=kernel -S < %s -o %t.table
...
; CHECK-ESIMD-IR-3-DAG: define dso_local spir_kernel void @ESIMD_kernel1()
; CHECK-ESIMD-IR-3-DAG: declare dso_local spir_func i64 @_Z28__spirv_GlobalInvocationId_xv()

; CHECK-ESIMD-IR-2-DAG: define dso_local spir_kernel void @ESIMD_kernel2()
; CHECK-ESIMD-IR-2-DAG: declare dso_local spir_func i64 @_Z28__spirv_GlobalInvocationId_xv()
